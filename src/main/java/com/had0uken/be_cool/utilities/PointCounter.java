...
public class PointCounter {
    ...
    private int getPoints(Type type, Status status){
        int k=0;
        if(status.equals(Status.FINISHED))k=1;
        if(status.equals(Status.FAILED))k=-1;

        switch (type) {
            case DAILY -> {return k*DAILY_POINTS;}
            case WEEKLY -> {return k*WEAKLY_POINTS;}
            case MONTHLY -> {return k*MONTHLY_POINTS;}
            case YEARLY -> {return k*YEARLY_POINTS;}
        }
        return 0;
    }
